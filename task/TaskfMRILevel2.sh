...
for LevelOnefMRIName in $LevelOnefMRINames ; do 
  ...
  LevelOneFEATDirSTRING="${LevelOneFEATDirSTRING}${ResultsFolder}/${LevelOnefsfName}${TemporalFilterString}${SmoothingString}_level1${RegString}${ParcellationString}.feat "; # space character at end is needed to separate multiple FEATDir strings
done
...
LevelTwoFEATDir="${ResultsFolder}/${LevelTwofsfName}${TemporalFilterString}${SmoothingString}_level2${RegString}${ParcellationString}.feat"
...
cat ${ResultsFolder}/${LevelTwofsfName}_level2.fsf > ${LevelTwoFEATDir}/design.fsf
