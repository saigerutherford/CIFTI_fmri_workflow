...
main() {
    ...
    taskfile=${StudyFolder}/${Subject}/${Subject}/func/${Subject}_task-${taskname}_space-fsLR_den-91k_bold.dtseries.nii
    ...
    FMRI_NPTS=`wb_command -nifti-information ${taskfile} | grep -w 'dim5' | awk '{print $2}'`

    # figure out where the template FSF file is
    fsf_template_file=${templatedir}/${taskname}_level1.fsf

    # copy the template file to the intended destination FSF file
    cp -p ${fsf_template_file} ${outdir}/${taskname}_level1.fsf

    # modify the destination by putting in the correct number of time points
    sed -i "s/fmri(npts) [0-9]*/fmri(npts) ${FMRI_NPTS}/" ${outdir}/${taskname}_level1.fsf

    echo ""
    echo "Level 1 FSF file generated at: "
    echo "  ${outdir}/${taskname}_level1.fsf"
    echo ""
}
