...
#log_Msg "READ_ARGS: Session: ${Session}" 
...
TR_vol=`${CARET7DIR}/wb_command -file-information ${ResultsFolder}/${Subject}_${LevelOnefMRIName}.dtseries.nii -no-map-info -only-step-interval`
...
FEATDir="${ResultsFolder}/${LevelOnefsfName}${TemporalFilterString}${SmoothingString}_level1${RegString}${ParcellationString}.feat"
...
cp /net/pepper/ivy_data/workflow/results/${Subject}/${Subject}/${LevelOnefsfName}_level1.fsf ${FEATDir}/design.fsf
...
#calculate number of frames to trim from CIFTI
trim=`grep "set fmri(ndelete)" ${FEATDir}/design.fsf | cut -d " " -f 3 | sed 's|"||g'`;
start=`expr ${trim} + 1`
end=${npts}
#trim CIFTI
log_Msg "MAIN: TRIM: Removing (""${trim}"") points from begining of run"
${CARET7DIR}/wb_command -cifti-merge ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_trim.dtseries.nii -cifti ${ResultsFolder}/${Subject}_${LevelOnefMRIName}.dtseries.nii -column ${start} -up-to ${end}
...
if [ "$Confound" != "NONE" ] ; then
	confound_matrix=$( ls -d ${ResultsFolder}/${Subject}_${Confound} 2>/dev/null )
fi
...
if $runDense ; then
	if [ "$FinalSmoothingFWHM" -gt "$OriginalSmoothingFWHM" ] ; then
		...
		${CARET7DIR}/wb_command -cifti-smoothing ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_trim.dtseries.nii ${AdditionalSigma} ${AdditionalSigma} COLUMN ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}.dtseries.nii -left-surface ${DownSampleFolder}/${Subject}.L.midthickness.${LowResMesh}k_fs_LR.surf.gii -right-surface ${DownSampleFolder}/${Subject}.R.midthickness.${LowResMesh}k_fs_LR.surf.gii
	else
		...
		cp ${ResultsFolder}/${Subject}_${LevelOnefMRIName}.dtseries.nii ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}.dtseries.nii
	fi
fi
...
if [[ $runParcellated == true || $runDense == true ]]; then
	...
	${CARET7DIR}/wb_command -cifti-convert -to-nifti ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}.${Extension} ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}_FAKENIFTI.nii.gz
	# Save mean image
	fslmaths ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}_FAKENIFTI.nii.gz -Tmean ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}_FAKENIFTI_mean.nii.gz
	...
	fslmaths ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}_FAKENIFTI.nii.gz -bptf ${hp_sigma} -1 \
	   -add ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}_FAKENIFTI_mean.nii.gz \
	   ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}_FAKENIFTI.nii.gz
	# Convert "fake" NIFTI back to CIFTI
	${CARET7DIR}/wb_command -cifti-convert -from-nifti ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}_FAKENIFTI.nii.gz ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}.${Extension} ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${TemporalFilterString}${SmoothingString}${ParcellationString}.${Extension}
	# Cleanup the "fake" NIFTI files
	rm ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}_FAKENIFTI.nii.gz ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${SmoothingString}${ParcellationString}_FAKENIFTI_mean.nii.gz
fi
...
if $runDense ; then
	...
	${CARET7DIR}/wb_command -cifti-separate-all ${ResultsFolder}/${Subject}_${LevelOnefMRIName}_${TemporalFilterString}${SmoothingString}.dtseries.nii -volume ${FEATDir}/${LevelOnefMRIName}_AtlasSubcortical${TemporalFilterString}${SmoothingString}.nii.gz -left ${FEATDir}/${LevelOnefMRIName}${TemporalFilterString}${SmoothingString}${RegString}.atlasroi.L.${LowResMesh}k_fs_LR.func.gii -right ${FEATDir}/${LevelOnefMRIName}${TemporalFilterString}${SmoothingString}${RegString}.atlasroi.R.${LowResMesh}k_fs_LR.func.gii
	...
	for Subcortical in ${FEATDir}/SubcorticalVolumeStats/*nii.gz ; do
		...
		${CARET7DIR}/wb_command -cifti-create-dense-timeseries ${FEATDir}/GrayordinatesStats/${File}.dtseries.nii -volume $Subcortical $ROIsFolder/Atlas_ROIs.${GrayordinatesResolution}.nii.gz -left-metric ${FEATDir}/L_SurfaceStats/${File}.func.gii -roi-left ${ROIsFolder}/L.atlasroi.${LowResMesh}k_fs_LR.shape.gii -right-metric ${FEATDir}/R_SurfaceStats/${File}.func.gii -roi-right ${ROIsFolder}/R.atlasroi.${LowResMesh}k_fs_LR.shape.gii
	done
